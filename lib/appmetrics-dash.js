...
function startApi(app) {
  ...
  app.get('/appmetrics/api/v1/collections', function(req, res) {
    try {
      ...
      for (var i = 0; i < collections.length; i++) {
        let string = fullUrl + '/' + collections[i].collection.id;
  ...
  app.post('/appmetrics/api/v1/collections', function(req, res) {
    try {
      ...
      res.status(201);
      let colUrl = fullUrl + "/" + col.collection.id;
...
function getCollection(req, res, cb) {
  ...
  if (index != -1) {
    try {
      ...
      col.endTime = new Date().getTime();
      col.duration = (col.endTime - col.startTime);
      if (cb) {
        cb(index);
      }
    } catch (err) {
