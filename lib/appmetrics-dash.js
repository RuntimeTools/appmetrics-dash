...
exports.monitor = function(options) {
  ...
  monitoring.on('mysql', function(data) {
    while (latestMysqlQueries.length>300){
      latestMysqlQueries.shift();
    }
    latestMysqlQueries.push({query:data.query,duration:data.duration});
...
function emitData() {
  ...
  if (latestMysqlQueries){
    io.emit('mysql', JSON.stringify(latestMysqlQueries));
  }
