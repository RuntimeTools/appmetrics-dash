...
exports.monitor = function(options) {
  ...
  monitoring.on('workpool', function(data) {
    latestWorkPoolEvent = data;
  });
...
function emitData() {
  ...
  if (latestWorkPoolEvent) {
    io.emit('workpool', JSON.stringify(latestWorkPoolEvent));
    latestWorkPoolEvent = null;
  }
  if (aggregateHttpEvent) {
