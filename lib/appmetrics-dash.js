...
exports.monitor = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    socket.emit(
      'status',
      JSON.stringify({ profiling_enabled: profiling_enabled })
    );
  ...
  monitoring.on('http', function(data) {
    if (!aggregateHttpEvent) {
      ...
      aggregateHttpEvent.average =
        (aggregateHttpEvent.average * (aggregateHttpEvent.total - 1) +
          data.duration) /
        aggregateHttpEvent.total;
  ...
  monitoring.on('https', function(data) {
    if (!aggregateHttpsEvent) {
      ...
      aggregateHttpsEvent.average =
        (aggregateHttpsEvent.average * (aggregateHttpsEvent.total - 1) +
          data.duration) /
    ...
    rest.updateCollections('https', data);
  });
  ...
  monitoring.on('http-outbound', function(data) {
    if (!aggregateHttpOutboundEvent) {
      ...
      aggregateHttpOutboundEvent.average =
        (aggregateHttpOutboundEvent.average *
          (aggregateHttpOutboundEvent.total - 1) +
          data.duration) /
        aggregateHttpOutboundEvent.total;
  ...
  monitoring.on('https-outbound', function(data) {
    if (!aggregateHttpsOutboundEvent) {
      ...
      aggregateHttpsOutboundEvent.average =
        (aggregateHttpsOutboundEvent.average *
          (aggregateHttpsOutboundEvent.total - 1) +
          data.duration) /
        aggregateHttpsOutboundEvent.total;
...
function addProbeEvent(probename, data) {
  ...
  for (var i = 0; i < aggregateProbeEvents.length; i++) {
    if (aggregateProbeEvents[i].name === probename) {
      ...
      aggregateProbeEvents[i].duration =
        (aggregateProbeEvents[i].duration * (total - 1) + data.duration) /
        total;
    }
  }
  if (!found) {
    aggregateProbeEvents.push({
      name: probename,
      total: 1,
      duration: data.duration,
      time: data.time
