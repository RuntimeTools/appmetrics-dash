...
// Buffer 1 cpu and 1 memory event
var latestCPUEvent;
var latestMemEvent;

// Number of data items to keep per type of data
var maxDataToKeep = 500;
// Interval between emitting data in milliseconds
var emitInterval = 5000;
...
exports.monitor = function(options) {
  ...
  if ('nodereport' in options) {
    ...
    try {
      nodereport = require('nodereport/api');
    } catch (err) {
  ...
  monitoring.on('gc', function(data) {
    gcBuffer.push(data);
    if(gcBuffer.length > maxDataToKeep) {
        gcBuffer.shift();
    }
  });
  ...
  monitoring.on('http', function(data) {
    ...
    if(httpBuffer.length > maxDataToKeep) {
        httpBuffer.shift();
    }
  });
  ...
  monitoring.on('http-outbound', function(data) {
    ...
    if(httpOutboundBuffer.length > maxDataToKeep) {
        httpOutboundBuffer.shift();
    }
  });
  ...
  setInterval(emitData, emitInterval);
  return monitoring;
};

function addProbeEvent(event) {
    probeEventsBuffer.push(event);
    if(probeEventsBuffer.length > maxDataToKeep) {
        probeEventsBuffer.shift();
    }
}
