...
function resizeCPUChart() {
    ...
    xAxis = d3.svg.axis().scale(x)
	    .orient("bottom").ticks(4).tickFormat(d3.time.format("%H:%M:%S"));
    chart.select(".processlatest").attr("x", graphWidth / 2) // change the text
                                                              // location
    chart.select(".processlatestlabel").attr("x", graphWidth / 2) // change the
                                                                  // text
                                                                  // location
    chart.select(".grid").call(make_y_axis()
			.tickSize(-graphWidth, 0, 0)
			.tickFormat(""))
}
